...
class BGESmallEmbeddingFunction:
    ...
    def encode_text(self, texts):
        ...
        embeddings = self.model.encode(texts, normalize_embeddings=True, batch_size=16, convert_to_numpy=True)
...
class SimpleKnowledgeBase:
    ...
    def __init__(self, milvus_uri: str = "http://localhost:19530"):
        ...
        self.embedding_function = BGESmallEmbeddingFunction(model_name="BAAI/bge-small-zh-v1.5", device="cpu")
    ...
    def load_sql_examples(self, data_dir: str):
        ...
        default_examples = [
            ...
            {
                "question": "查询用户订单信息",
                "sql": "SELECT u.name, p.name, o.quantity FROM orders o JOIN users u ON o.user_id = u.id JOIN products p ON o.product_id = p.id",
                "description": "订单详情",
                "database": "sqlite",
            },
            {
    ...
    def vectorize_and_store(self):
        ...
        for schema in self.table_schemas:
            columns_desc = ", ".join(
                [f"{col['name']} ({col['type']}): {col.get('description', '')}" for col in schema["columns"]]
            )
    ...
    def add_sql_example(self, question: str, sql: str, description: str = ""):
        """添加新的SQL示例"""
        new_example = {"question": question, "sql": sql, "description": description, "database": "sqlite"}
        ...
        if self.collection and self.data_loaded:
            ...
            insert_data = [["sql_example", question, sql, description, "", embedding.tolist()]]
