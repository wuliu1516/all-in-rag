...
class HybridMultimodalEncoder:
    ...
    def __init__(self, visual_model_name: str, visual_model_path: str):
        ...
        # 初始化BGE-M3模型（用于混合检索）
        model_path = (
            "/home/app/.cache/huggingface/hub/models--BAAI--bge-m3/snapshots/5617a9f61b028005a4858fdac845db406aefb181"
        )
        self.bge_m3 = BGEM3EmbeddingFunction(model_name=model_path, use_fp16=False, device="cpu")
    ...
    def encode_text_hybrid(self, text: str) -> dict:
        ...
        return {"sparse": embeddings["sparse"], "dense": embeddings["dense"]}
...
def visualize_results(
    ...
    if query_image_path and os.path.exists(query_image_path):
        ...
        cv2.putText(
            query_display_area, "Query", (10, panoramic_height - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2
        )
        cv2.putText(
            query_display_area, search_mode, (10, panoramic_height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 100, 0), 2
        )
...
class HybridMultimodalSearcher:
    ...
    def __init__(self, data_dir: str, metadata_path: str, collection_name: str, milvus_uri: str):
        ...
        self.encoder = HybridMultimodalEncoder(
            visual_model_name="BAAI/bge-base-en-v1.5", visual_model_path="../../models/bge/Visualized_base_en_v1.5.pth"
        )
    ...
    def search(self, query_image_path: str, query_text: str, mode: str = "hybrid", top_k: int = 5) -> list:
        ...
        if mode == "multimodal":
            ...
            results = self.collection.search(
                [query_vector], "multimodal_vector", param=cosine_params, limit=top_k, output_fields=output_fields
            )[0]
            ...
            results = self.collection.search(
                [dense_vec], "text_dense_vector", param=search_params, limit=top_k, output_fields=output_fields
            )[0]
            ...
            results = self.collection.search(
                [sparse_vec], "text_sparse_vector", param=search_params, limit=top_k, output_fields=output_fields
            )[0]
    ...
    def visualize_comparison(self, query_image_path: str, query_text: str, top_k: int = 3):
        ...
        for mode in modes:
            ...
            retrieved_results = []
            for hit in results:
                retrieved_results.append({"image_path": hit.entity.get("image_path"), "distance": hit.distance})
